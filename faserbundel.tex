...
\label{eq:kommut}
\end{equation}
gilt, nennen wir $\left( E, \pi, B ; F\right)$ \textbf{Faserbündel} mit \textbf{typischer Faser} $F$. Nach \autoref{eq:kommut} kommutiert also folgendes Schema:
\begin{center}
...
\thmbox{lemdef}{Rückzug}{\label{def:ruck} Seien $M, N$ \difmans, und $f \colon M \to N$ eine glatte Abbildung. Für ein Faserbündel $(E, \pi, M)$ definieren wir $f^*(E, \pi, M) := (f^*E, \tilde \pi, N)$ durch:
\begin{align*}
...
\TODO{ausarbeiten}
\begin{labeling}{\hspace*{35mm}$\Gamma(\Omega^k T^*M)$}
 \item[\hspace*{35mm}$\Gamma(TB)$]		Vektorfelder über $B$,
 \item[\hspace*{35mm}$\Gamma(T^*B)$]		1-Formen über $B$,
 \item[\hspace*{35mm}$\Gamma(\Omega^k T^*M)$]	k-Formen über $B$.
\end{labeling}
}

\thmbox{defbem}{Vektorbündel}{Für ein beliebiges Faserbündel $(E, \pi, B)$ mit typischer Faser $F$, kann die Menge der Schnitte $\Gamma(E) = \emptyset$ sein.

Existiert jedoch ein Isomorphismus $F \to \K^n$, so gibt es immer Schnitte. \todo{Nullschnitt?} Ein solches Faserbündel nennen wir auf $\K$-Vektorbündel.}
